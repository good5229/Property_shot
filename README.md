...
| 4. 풍선 | 뾰족함으로 풍선을 터뜨려 ? 상자의 기믹 드러내기 |
...
- 사람의 재미 목표 → Codex 후보 → Validator 반려·통과 → 사람 채택과 실제 반려 사례를 앱에서 보여 주는 `Codex Puzzle Forge`
- 홈의 `EN/한국어` 전환을 저장하고 홈·60초 핵심 체험·Puzzle Forge·3장면 일일 도전의 심사 핵심 문안을 한·영으로 제공
- 저장 읽기·쓰기 실패와 지연 복원은 현재 플레이를 중단하거나 최신 사용자 선택을 되돌리지 않으며, 320px·글자 200%·1440px Web 경로를 회귀 검증
- OpenAI Game Builders Track 1 제출 문안·신규 개발 manifest·16:9 썸네일은 `submission/openai_game_builders_track1/`에서 함께 관리
...
- 오늘의 도전은 KST 날짜마다 검증된 생산 장면 3개만 열고, 네 항로가 스테이지 10종을 순환
- 외부 플랫폼은 `GamePlatformGateway` 뒤에 두며 현재는 로그인 없는 기기 기록만 사용하고 Hive 연동 완료를 주장하지 않음
