...
Web Release 대리 측정의 발사 구간 p90이 17.1~17.3ms로 엄격 목표 16.7ms를 0.4~0.6ms 초과했다. 발사 p99 25ms 이하와 발사 중 50ms 초과 0건은 충족했지만, 390×844 유휴 구간에서 50.6ms 프레임 1건이 관측됐다. 실제 iPhone·Android·iPad 측정이 없으므로 최종 판정은 `엄격 성능 게이트 미통과·실기기 미검증`이다.
...
|---|---|---:|---:|---:|---:|---:|---:|
| 390×844 | idle | 16.972ms | 17.1ms | 17.2ms | 17.6ms | 50.6ms | 1건 |
| 390×844 | 발사 | 16.780ms | 17.1ms | 17.3ms | 17.6ms | 32.9ms | 0건 |
| 768×1024 | idle | 16.667ms | 17.1ms | 17.3ms | 17.5ms | 17.6ms | 0건 |
| 768×1024 | 발사 | 16.778ms | 17.3ms | 17.5ms | 17.7ms | 32.7ms | 0건 |

- 두 뷰포트 모두 콘솔 오류 0건, 측정된 발사 구간 Long Task 0건이다. 390×844 유휴 프레임의 50.6ms 1건은 실패 표본에서 제외하지 않았다.
...
| 목표 | 결과 | 판정 |
|---|---|---|
| 발사 p90 ≤ 16.7ms | 17.1ms·17.3ms | 미통과 |
| 발사 p99 ≤ 25ms | 17.6ms·17.7ms | 통과 |
| 발사 중 50ms 초과 0건 | 두 뷰포트 0건 | 통과 |
| 일반 구간 50ms 초과 0건 | 390×844 유휴 1건 | 미통과 |
