...
기준 브랜치: `main`
기준 커밋: `2061280e616d790a7ecfb0571b42ecfc3ce589e1`

## 최종 제출 자동 회귀 재확인

- 실행 시각: 2026-08-09 KST
- 명령: `flutter test --concurrency=1`
- 결과: **988개 통과, 실패 0**, 4분 58초
- 추가 결과: `flutter analyze` 문제 0건, Web Release 및 Android Release APK(62.3MB) 빌드 통과
- 아래의 898개·901개 수치는 기능 추가 과정의 과거 실행 기록이다. 최신 제출 자동 회귀는 988개 결과를 기준으로 한다.
- 이 자동 결과는 Web rAF 대리 측정이나 Android 에뮬레이터 증거와 구분하며, 실제 모바일 성능 게이트를 통과시키지 않는다.

## 최신 판정

Web Release 3회 반복 대리 측정의 발사 입력 구간 rAF p90은 17.9~18.1ms로 엄격 목표 16.7ms를 초과했다. 두 해상도 모두 발사 입력 구간 p99 25ms 이하, 누락 프레임·50ms 초과·Long Task·콘솔 오류 0건은 충족했다. 다만 rAF 간격은 브라우저 표시 스케줄러 흔들림을 포함하고 Flutter build/raster 실행 시간이 아니며 실제 iPhone·Android·iPad 측정도 없으므로 최종 판정은 `엄격 성능 게이트 미통과·실기기 미검증`이다.
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 390×844 | 유휴 | 325 | 16.775ms | 16.7ms | 18.1ms | 18.2ms | 18.6ms | 33.5ms | 2건 | 0건 |
| 390×844 | 발사 입력 | 568 | 16.662ms | 16.7ms | 18.1ms | 18.2ms | 18.6ms | 18.7ms | 0건 | 0건 |
| 768×1024 | 유휴 | 327 | 16.672ms | 16.7ms | 18.0ms | 18.1ms | 18.5ms | 18.7ms | 0건 | 0건 |
| 768×1024 | 발사 입력 | 569 | 16.670ms | 16.7ms | 17.9ms | 18.2ms | 18.5ms | 18.6ms | 0건 | 0건 |

- 두 뷰포트의 3회 모두 콘솔 오류와 유휴·발사 입력 Long Task가 0건이다.
- 누락 프레임은 각 구간 중앙값의 1.5배를 초과한 rAF 간격으로 정의했다. 390×844 유휴에서 2건이 있었지만 발사 입력 구간은 두 해상도 모두 0건이다.
...
| 목표 | 결과 | 판정 |
|---|---|---|
| 발사 입력 rAF p90 ≤ 16.7ms | 18.1ms·17.9ms | 미통과 |
| 발사 입력 rAF p99 ≤ 25ms | 18.6ms·18.5ms | 통과 |
| 발사 입력 누락 프레임 0건 | 두 뷰포트 0건 | 통과 |
| 전체 측정 50ms 초과 0건 | 유휴·발사 입력 모두 0건 | 통과 |
| 콘솔 오류 0건 | 두 뷰포트 0건 | 통과 |
| 실제 모바일 기기 확인 | 증거 없음 | 미검증 |

rAF p90 미통과만으로 앱 렌더 작업이 16.7ms를 넘었다고 단정하지 않는다. 중앙값 16.7ms에 브라우저 콜백 간격이 양쪽으로 흔들리면서 평균은 약 16.67ms를 유지했고 발사 입력 구간의 누락 프레임은 없었다. Flutter build/raster 시간은 DevTools 또는 실기기 profile 측정으로 별도 판정해야 한다.
...
- Web rAF p90 17.9~18.1ms가 브라우저 표시 스케줄러 흔들림인지 실제 렌더 비용인지 Flutter DevTools·실기기 profile로 분리
