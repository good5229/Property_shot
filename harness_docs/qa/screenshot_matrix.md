...
- 콘솔 오류는 390×844·768×1024 모두 0건이지만, 발사 평균·p95는 각각 25.419·66.6ms와 42.222·150ms이고 20ms 초과 비율은 각각 25.25%·85.00%였다.
- 빈 Chromium 기준은 두 해상도 모두 평균 약 16.7ms·p95 18.6ms다. 따라서 최신 게임 번들의 성능 게이트는 통과로 기록하지 않는다.
- 256px 런타임 래스터 디코드와 이미지 수명 해제는 유지했지만, Picture 캐시 최적화는 Golden 또는 성능 회귀로 제거했다. 발사 중 재질·이펙트·래스터·Flutter Web 합성 비용이 남아 있다. 이 증거는 실제 iPhone·iPad 성능을 대신하지 않는다.
