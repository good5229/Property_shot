...
## 2026-08-03 단계4 실제 조작 재생

- `scripts/capture_stage4_web_evidence.py`로 390×844 Release Web의 단계4 실제 흐름을 재생했다.
- 속성 팝업·속성 이전·조준·충전·발사 후 충돌 시점·결과를 8장으로 저장했다.
- `screenshots/commercial-vertical-slice/stage4-web-evidence.json`의 콘솔 오류는 0건이다.
- 성공 상태에서 `active_ball`이 제거된 뒤 발생하던 HUD·telemetry 예외를 `07ef7b3`에서 수정했다.
...
- 콘솔 오류는 390×844·768×1024 모두 0건이지만, 발사 평균·p95는 각각 24.195·51.5ms와 38.717·66.6ms이고 20ms 초과 비율은 각각 18.27%·80.00%였다.
- 빈 Chromium 기준은 두 해상도 모두 평균 약 16.7ms·p95 18.6ms다. 따라서 최신 게임 번들의 성능 게이트는 통과로 기록하지 않는다.
- 256px 런타임 래스터 디코드·이미지 수명 해제·정적 물체 Picture 캐시·중간 품질 필터를 유지했다. 보드 래스터 캐시는 검증되지 않아 제거했으며, 발사 중 재질·이펙트·래스터·Flutter Web 합성 비용이 남아 있다. 이 증거는 실제 iPhone·iPad 성능을 대신하지 않는다.
...
## 2026-08-07 7단계 과거 공 화면 기준

- `test/goldens/game_screen_stage7_{390x844,393x852,430x932,768x1024,1024x1366}.png`는 7단계 시작 화면의 한글 목표, 보드, 공, 상자, 벽, 홀과 발사 조작 영역을 고정한다.
- `test/goldens/stage7_persistent_{390x844,393x852,430x932,768x1024,1024x1366}.png`는 점착으로 고정된 첫 번째 공의 순번, 속성 설명, 고정 상태와 다음 충돌 역할을 팝업으로 고정한다.
- `stage_select_390x844.png`와 `stage_select_768x1024.png`는 7단계 카드, 잠금 상태와 게임 공 얼굴 아이콘을 포함하도록 갱신했다.
- 다섯 화면 크기를 직접 확인해 한글 잘림, 팝업·버튼 겹침, 보드 이탈이 없음을 확인했다. Golden은 테스트 렌더 기준이며 실제 기기 성능과 사용자 이해도를 대신하지 않는다.
