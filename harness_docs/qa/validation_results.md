# 검증 결과

실행 일시: 2026-08-01 KST

| 명령 | 결과 |
|---|---|
| `dart format --output=none --set-exit-if-changed .` | 통과, 변경 없음 |
| `flutter analyze` | 통과, 이슈 없음 |
| `flutter test` | 통과, 75개 |
| `flutter build web --release` | 통과 |
| `curl http://127.0.0.1:8080` | HTTP 200, 새 빌드 서버 |
| `flutter build ios --no-codesign` | Xcode 빌드 완료 후 Development Team/프로비저닝 프로파일 부족으로 배포 빌드 실패 |

기존 8080 프로세스를 종료한 뒤 새 빌드 서버를 시작했고 HTTP 200 응답을 확인했다.

이번 검증에는 벽 네 방향 경계, 연쇄 벽 반사, 벽 충돌 법선 시각 피드백, 상태가 비고체여도 벽을 물리 장애물로 처리하는 불변식, 미리보기와 실제 벽 후보 일치, 비정사각형 원형 히트박스, 반발값에 따른 상자 이동량 차이, 2단계 직선 우회 차단, 3단계 점착 발판 선행과 진행 안내, 연쇄 스위치 점착 조건, 탄성 학습 단계의 일반 공 감속, 동일 질량 공 충돌, 점착 지속성, 홀 경계 성공·실패, 실패 복구 패널, 팝업 접근성 격리, 접근성 힘 조절, 320px 안내 말줄임 방지, 발사 중 정보 팝업 차단, 보드 우선 반응형 레이아웃 회귀가 포함되며 전체 75개 테스트가 통과했다.
