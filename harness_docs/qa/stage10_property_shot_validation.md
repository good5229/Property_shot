...
- 역사 기록: PS-STAGE-08 구현 당시 전체 회귀는 낡은 화면 Golden 9건 때문에 `+330/-9` 시점에 중단됐다. 이후 실제 화면 대조를 거쳐 Golden을 갱신했으며 2026-08-08 최신 전체 회귀 892건은 실패 0건이다. 당시 중단 수치는 현재 잔여 결함으로 해석하지 않는다.
...
- 스테이지 추가로 전체 자동 검증 시간은 늘었고 최신 직렬 회귀는 약 3분 37초가 걸렸다. 자동 전체 회귀는 통과했지만 실제 기기 플레이테스트는 여전히 별도 통합 단계가 필요하다.
