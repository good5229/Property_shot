...
현재 전체 회귀 기준은 244개 통과다. 이전 표의 231개는 당시 문서 시점의 수치로 보존하며 최신 종합 판정에는 사용하지 않는다. Android ARM64 에뮬레이터에서 동일한 기능 흐름을 재생했지만, Web·iOS·Android 실기기 간 이벤트 지문 동등성은 아직 검증하지 않았다.
...
| 실행 결과 | 결정론 3개, 충돌 행렬 1개, 다중샷 분석 1개, 저장 다중샷 재생 1개, 전체 회귀 231개 통과 |
