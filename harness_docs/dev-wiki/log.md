...
2026-08-03: 단계별 다중샷 픽스처를 점검한 결과 기존 20도·5단계 격자는 2단계와 4단계의 실제 2발 경로를 놓치고 있었다. 순수 `ShotResolver` 정밀 탐색에서 네 단계 모두 2발 성공 입력을 찾았으므로 생성 격자를 10도·10단계로 높이고 단계별 2발 존재 조건을 회귀에 추가한다.
