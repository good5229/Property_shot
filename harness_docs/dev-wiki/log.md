...
## [2026-09-02] physics | Alkkagi contact-island causality pass

- 알까기 영상의 연쇄 전달을 목표 동작으로 삼고 Box2D의 고정 하위 스텝·순차 임펄스 자료를 참고해, 가까운 동적 충돌들을 한 접촉군으로 묶는 10회 반복 속도 해석기를 추가했다. 첫 순회에서만 반발을 적용하고 뒤 순회는 새 접근 속도만 제거해 중간 물체의 역튕김과 에너지 증가를 막는다.
- 240Hz 화면 타임라인은 2 cursor 안의 연쇄 접촉을 함께 해석하고 질량·재질별 반발을 사용한다. 따라서 뒤 물체가 앞 물체의 전체 경로 종료를 기다리던 계단식 재생 대신 같은 시간대에 운동량을 받으며, 확정 판정 좌표와 30·45·60 FPS 보간 계약은 유지한다.
- Stage 8 보고서를 현재 resolver에서 직접 생성하는 공개 builder로 바꾸고 JSON과 완전 일치를 테스트한다. 네 패턴 모두 실제 성공 입력에서 서로 다른 인과 서명을 2개 이상 가졌으며, 각 패턴의 두 서명 이상이 연결 영역 2칸 이상을 유지한다.
- Stage 9 반사판 해법은 최종 발의 홀 인과 사슬에 기대 반사판 접촉이 있거나, 같은 둘째 발이 미준비 초기 상태에서는 실패하는 반사실 준비 효과가 있을 때만 집계한다. 직접 우회는 패턴 01에서 12개, 패턴 04에서 180개 제외했고 정상 준비 영역은 각각 1,056·74·102·198개를 보존했다.
- 집중 회귀 96건과 최종 접촉군·Stage 9 재검증 8건, 정적 분석 0건, 1,200회 성능 probe(p95 12.186ms, p99 22.089ms), GitHub Pages용 Wasm Web 빌드가 통과했다. 로컬 macOS의 기존 탄성 Golden은 기준 커밋에서도 홀 주변 1.45% 픽셀 차이가 나며 1440×900 테스트 러너가 정지해, Linux 구조 Golden을 포함한 전체 gate는 원격 Actions에서 최종 확인한다.

## [2026-08-26] qa | collision, loading, stage, restoration integration

- 성능·물리, 레벨·보상, UI·운영 역할의 웹 기반 Terra 감사 결과를 통합하고 공통 원인부터 수정했다. 물리 path index와 화면 표시 cursor를 분리해 3·5·8·12중 충돌을 최소 beat로 펼치고, 이동 위치·재질 변형을 실제 이동 거리 시간축으로 통일했다.
- Flutter 첫 프레임 전에도 엔진 준비·기록 로드·첫 화면 렌더 상태를 보여 주며, bootstrap 실패 때 접근 가능한 오류와 다시 시도를 남기는 로딩 페이즈를 추가했다.
- 생산 40패턴은 시작 공 주변 72px, 초기 시각 요소 비겹침, 공-홀 직선 통로 차단을 만족한다. 전 패턴의 기믹 사용 성공 영역이 우회보다 최소 1.4배 넓고, 연쇄 점수 패턴은 최소 1.3배다. 대표 입력·리플레이·기준 이미지와 고정 좌표 위젯 테스트를 새 실제 배치로 갱신했다.
- 15개 발견 실제 게임 상태에서 관측소 분석, 등대 L1·조준 지원, 다리 런당 복제 코어와 추천·집중 지원·중복 방지·저장 복원을 다시 확인했다.
- Sol 최종 감사에서 4단계 `?` 상자의 실제 정사각 렌더가 schema 직사각형보다 커 풍선과 6.37px 겹치지만 기존 감사가 놓치는 P1을 발견했다. Canvas와 감사의 미리보기 크기를 공통화하고 8px 시각 여백을 검사하며, 배치를 `?` 상자-풍선 9.34px·`?` 상자-문 12.34px로 분리했다. 과거 겹침 좌표는 음성 회귀로 고정했고 Sol 재감사는 P0/P1 없음으로 PASS했다.
- 최종 증거는 전체 1,483개 테스트, 정적 분석 이슈 0건, 생산 40패턴 생성·런타임 검사, Wasm Web 릴리스 빌드, diff 검사, 320×568~1920×1080 Golden, 로컬 브라우저 console warning/error 0건이다. 실제 사람의 체감과 저사양 실기기 성능은 자동 증거로 대체하지 않는다.
...
- 공모전용 소개 흐름은 첫 8초에 속성 선택과 기믹 인과를 보여 주고, 실제 탄성 다중 반사 플레이·섬 시설 복구·후반 기믹·지원 기능으로 이어지도록 재구성한다.
- 1–3단계는 배워야 할 속성과 첫 행동을 한 문장 코치마크로 좁히고 홀 수용 범위를 완화하되, 기믹 경로의 우위·무기믹 우회 상한·대표 입력 근방 성공 계약을 함께 유지한다.
- 발견 3·9·15개에서 관측소·등대·다리 복구 완료를 크게 알리고 분석 강화·L1 즉시·복제 코어 지원이 실제로 열린다는 점을 같은 연출에서 설명한다.
- 게임 소개서는 심사자가 핵심 루프, 차별점, 반복 플레이, 목표 고객, 수익 가설과 상용화 로드맵을 먼저 읽도록 재편하며 저장소·커밋·테스트 ID는 전면 서사에서 제외한다.
- 《산나비》의 완성도는 콘텐츠 양이 아니라 한 문장 핵심 동사, 고유한 세계 실루엣, 조작·연출·음악의 결합, 감정적 진행 이유로 분해해 벤치마크했다. 홈과 섬 지도에 관측소·등대·다리가 한 장면에 보이는 프로젝트 전용 섬 배경을 적용하고, 홈 문구도 조작 설명에서 시설을 깨우는 목적 설명으로 바꿨다. 2.1MB 생성 PNG는 시각 검수 후 177KB WebP로 최적화했으며 원본은 프로젝트 번들에서 제외했다.

## [2026-08-15] engagement-ux | PS-ENGAGE-02 start

- 사용자 승인 범위는 진행 저장을 오염하지 않는 물리 실험실, 에이전트 기반 대리 플레이테스트 프로토콜, 개별 도움 설정, 실제 `ShotResolver` 증거를 쓰는 실패 조언, Web 키보드·포커스 접근성이다.
- 탐사 계약 변주와 다수 실기기 확보를 전제로 한 검증은 이번 범위에서 제외한다. 에이전트 평가는 사람의 손맛·감정·재미를 대체하지 않고 동선·문구·상태 복구의 대리 평가로만 판정한다.
- 구현 뒤 물리 결정론, 캠페인·오늘의 도전 진행 비오염, 도움 사용 공정성, 반사실 조언의 인접 입력 안정성, 비정상 입력과 키보드 포커스 회귀를 별도 검증한다.
...
- 기존 `input_latency_ms`가 포인터 해제와 무관하게 순수 `ShotResolver` 실행 시간만 기록하는 계약 불일치를 확인했다.
- 포인터에서 손을 뗀 순간을 단조 시계로 표식하고, 비동기 발사 저장과 물리 판정이 끝날 때까지를 샷 입력 처리 지연으로 기록한다.
- 37.5ms 단위 계산·시계 역행 하한과 30ms 지연 저장을 통과하는 실제 게임 화면 회귀를 추가했다.
...
## 2026-08-09 입력 지연 p95 판정 자동화

- 현재 앱 세션의 비리플레이 `shot_released`만 집계해 최소 20표본의 최근접 순위 p95를 계산하는 `InputLatencyReport`를 추가했다.
- p95 50밀리초 이하만 통과하며, 20표본 미만은 표본 부족으로 유지하고 리플레이·비유한 값·음수·잘못된 타입은 판정 표본에서 제외한다.
- 개발 진단 메뉴에 현재 표본 수와 판정을 한글로 표시하고 보고서 JSON을 복사하는 행동을 추가했다. 과거 실행의 교정 전 값은 현재 세션 판정에 섞지 않는다.
- 텔레메트리·진단 메뉴·전체 게임 화면 집중 회귀 98개와 정적 분석 문제 0건을 확인했다.
- 전체 Flutter 회귀 898개가 약 4분 2초에 통과했다.
- Web Release와 Wasm 사전검사가 통과했고 `입력 지연 보고서 복사`·`개발 진단 메뉴` 문구가 Release 번들에서 제거됨을 확인했다. 앱 번들 해시는 직전 감사와 같은 `ccf9b216...cd59`이며 기존 PID 796을 종료하고 최신 PID 21973으로 교체해 루트·앱·음악 HTTP 200을 확인했다.

## 2026-08-09 Web 성능 감사 계약 교정

- 문서상 5초·5초와 달랐던 실제 기본 워밍업 1.8초·1.2초를 5초·5초로 맞추고, 초기 로딩의 buffered Long Task가 유휴 측정에 섞이지 않게 했다.
- 공을 누르기 직전부터 충전 0.65초와 손을 뗀 뒤 2.5초까지 프레임을 수집하며, 뷰포트별 새 브라우저 컨텍스트 3회와 전체 표본 집계를 함께 저장한다.
- 최신 390×844·768×1024 발사 입력 rAF p90은 18.1ms·17.9ms, p99는 18.6ms·18.5ms이며 발사 누락 프레임·50ms 초과·Long Task·콘솔 오류는 0건이다.
- rAF 간격은 표시 스케줄러 흔들림을 포함하고 Flutter build/raster 시간이 아니므로 엄격 p90은 미통과로 유지하되 앱 렌더 비용으로 단정하지 않는다. 접근성 상태를 통한 시도 증가 확인은 후속으로 남긴다.

## 2026-08-15 PS-ENGAGE-02 물리 실험실·적응형 재시도·Web 입력

- 섬 지도에 캠페인 기록과 분리된 5개 물리 실험을 추가했다. 속성은 실제 원본에서 옮기며, 발사·초기화·나가기 후에도 진행·탐사·일일 도전·도감·리플레이·보상 저장이 byte-for-byte로 변하지 않는다.
- 실패 후 실제 발사 입력 주변의 각도 6개·힘 6개만 재판정해 한 축의 강건한 변경만 문장으로 제안한다. 기존 기믹 성과 집합을 보존하지 못하는 후보, 단일 운좋은 후보, 예외·비유한 경로는 거부하며 각도·힘 수치는 UI에 노출하지 않는다.
- Web 보드에 포커스·화살표·Space·Escape 입력을 추가했다. 키보드 입력 중에만 3px 고대비 테두리를 캔버스 앞에 그리고, 반복 Space·충전 취소·팝업 배경 입력 차단을 회귀로 고정했다.
- 설정의 난이도 표현을 `예상 첫 도착 표시`로 바꾸고 최고 기록·선택 도전 분리를 설정 안에서 바로 알린다. 320×568·390×844·768×1024 Golden에서 스크롤·줄바꿈·읽기 흐름을 확인했다.
- 생산 40패턴의 반사실 조언 결정론과 실험 초기 공–홀 비중첩·목표 사건 도달성, 저장 격리, 세 해상도 지도·설정·실험실 Golden을 포함한 전체 Flutter 회귀 1,160개가 4분 45초에 통과했다. 대리 플레이 프로토콜·schema는 검증했지만 연결 가능한 브라우저가 없어 실제 agent 파일럿 점수는 생성하지 않았다.
# 2026-08-16 · 제21회 경기게임오디션 일반부문 경쟁력 보강

- 최초 참가 가점 3점 대상임을 확인했다.
- 공모전 평가축인 기획·재미·완성도·시장성·출시 가능성에 맞추어 1~3단계 첫 학습 흐름, 섬 복구 보상 연출, 게임 중심 소개서·시장/출시 계획, 60초 플레이 영상을 갱신한다.
- 기존 사용자 포트폴리오 수정 3건은 이 작업의 소유 범위가 아니므로 보존한다.
2026-08-16 | 《산나비》 완성도 벤치마크의 시각 격차 중 카메라·파티클·재질·광원·외곽선을 우선 보강했다. 강한 충돌은 접점 중심의 결정론적 카메라 펀치, 속성별 파편, 홀 수렴광으로 구분하고, 모든 기물과 공에 공통 실루엣·왼쪽 위 림라이트를 적용했다. 모션 감소에서는 카메라 펀치를 끄고 파티클 수를 줄인다. 물리 좌표·히트박스·ShotResolver는 변경하지 않았다.
2026-08-16 | 새 탄성 충돌 VFX 전용 Golden과 모바일·태블릿 전체 시각 기준선을 검수했다. `flutter analyze`, 생성 카탈로그 2종 동기화, 집중 53개 테스트, 전체 직렬 1,199개 테스트와 Web release 빌드가 통과했다.
2026-08-17 | 공 꾸미기 보상이 생성 스프라이트 경로에서는 외곽 링만 덧그려져 본체 변화가 약했던 원인을 수정했다. 청록 유광 본체 오버레이, 청록·금빛 이중 테두리, 하이라이트, 미세 파티클과 발사 잔상을 현재·과거·이동 공에 적용하되 엔티티 크기와 ShotResolver 입력은 바꾸지 않았다. 모션 감소는 회전·맥동을 없애고 고정 파티클 세 점만 유지한다. 실제 자산을 불러온 390×844·768×1024·1440×900 Golden, 세 표시 크기·모션 감소 Golden, 물리 불변·선택 팝업·선택/복원 집중 46건 및 정적 분석이 통과했다.
2026-08-23 | OpenAI Game Builders Track 1의 첫 구현 단위로 캠페인 저장과 분리된 `60초 핵심 체험`을 홈 최상위 행동에 추가했다. 생산 카탈로그의 `stage_heavy_01`·`stage_drained_01`·`stage_persistent_01`을 무거움 이전→비워진 원본의 양면 변화→남은 공 재사용 순서로 연결하고, 장면별 목표·순서·완료 행동을 기존 GameScreen에 표시한다. 초행 홈의 중복 첫 임무 카드를 제거해 320×568에서도 핵심 체험·캠페인·지도 행동을 첫 화면에 유지한다. 체험은 `GameProgressPersistencePolicy.disabled`로 진행·보상·도감을 오염시키지 않는다.
2026-08-23 | 속성 이전을 공의 강화로만 보이던 표현을 양면 변화로 확장했다. 원본이 잃은 속성을 `EntityState.drainedTraits`에 명시적으로 보존하고, 이전 직후 원본 `보유→비움`·공 `기본→획득`·원본의 물리 변화를 생성 이미지와 문구로 3초간 함께 보여 준다. 비워진 원본에는 색과 무관한 취소선 표식을 그리고 팝업·스크린 리더도 같은 상태를 설명한다. 무거움·탄성·점착·뾰족함 도메인/물리/리플레이 회귀와 390×844·768×1024 Golden을 통과했다.
2026-08-23 | 홈의 행동을 초행·첫 클리어·3단계 학습 상태로 나눠 점진 공개했다. 첫 클리어 직후 화면에 보상 버튼과 잠금 설명을 함께 늘어놓지 않고 `다른 활동`에 접었으며, 메뉴를 연 경우에만 보상과 정확한 다음 해금 조건을 보여 준다. 3단계 뒤에는 같은 위치에 리플레이와 오늘의 도전을 추가해 메뉴 위치 학습을 보존한다. 초행 6개 화면 크기, 중간 진행 390×844·768×1024, 숙련 4개 화면 크기 Golden과 실제 보상·리플레이·오늘의 도전 진입을 검증했다.
2026-08-23 | OpenAI 공모전의 Codex Collaboration을 앱 안에서 확인할 수 있는 `Codex Puzzle Forge`를 추가했다. 사람의 재미 목표→Codex 후보→StagePatternValidator 반려/통과→사람 채택을 모바일 세로·PC 가로 순서도로 표시한다. 생성 도구는 생산 40패턴의 정적·실제 ShotResolver 실행을 통과시킨 뒤, 시작 공–홀 겹침과 잘못된 문 연결 변이가 안정 코드로 검출되는 경우에만 제품 JSON을 갱신한다. 앱은 개발 도구나 테스트 fixture가 아니라 생성 요약만 읽는다. 홈 왕복·한국어 Semantics, Validator 정적/런타임, 390×844·768×1024·1440×900 Golden을 포함한 집중 70개가 통과했다.
2026-08-23 | OpenAI Game Builders Track 1 제출 패키지를 추가했다. 기존 기반과 챌린지 기간 신규 개발을 manifest로 분리하고, 200자 이하 소개·무로그인 플레이 링크·Codex 협업 설명을 복사 가능한 한국어 문안으로 정리했다. 프로젝트 자체 비주얼만 참고한 1920×1080 PNG 썸네일은 생성·정밀 편집 프롬프트와 SHA-256을 함께 보존하며 크기·규격을 자동 검사한다. 3분 영상은 사용자 요청에 따라 보류하고 Hive 연동 완료를 주장하지 않는다.
2026-08-23 | 오늘의 도전을 날짜별 세 장면으로 단축했다. 네 항로가 생산 스테이지 10종을 모두 순환하고, 포함된 40패턴은 StagePatternValidator 정적 검사와 실제 ShotResolver 대표 해법을 다시 통과한다. 마지막 장면에서 네 번째 장면을 요청하지 않고 결과를 확정한다. 결과 저장은 게스트 우선 `GamePlatformGateway`에 연결했으며 현재 구현은 기기 기록만 저장하고 온라인 순위·클라우드 저장을 지원한다고 표시하지 않는다. 향후 Hive SDK가 Flutter Web에서 확인되면 SDK 타입을 게임 규칙에 유입하지 않고 이 구현만 교체한다.
2026-08-23 | 심사 핵심 경로에 저장되는 한국어·영어 전환을 추가했다. 홈의 핵심 행동, 60초 핵심 체험의 장면 목표·완료 요약, Puzzle Forge의 사람·Codex·Validator 역할과 반려 사례, 3장면 일일 도전의 개요·결과가 한 번의 전환으로 함께 바뀐다. 알 수 없는 저장 언어는 기본값으로 복구한다. 한국어 홈 320×568~1920×1080, 영어 홈·일일 도전 390×844·1440×900, 영어 Puzzle Forge 모바일과 기존 PC 흐름을 Golden·overflow로 검증했다.
2026-08-23 | OpenAI 제출 경로의 최종 복원력 감사를 완료했다. 언어 저장 읽기·쓰기 실패와 지연 복원 경쟁은 현재 세션과 최신 사용자 선택을 보존하고, 일일 결과는 실제 달력 날짜와 Web 안전 범위의 점수·발사 수만 허용한다. 320×568·글자 200%에서 발견한 지도 목표 카드 높이와 경로 헤더 넘침, 실제 320px Web에서 발견한 영문 제목 잘림을 수정하고 별도 Golden으로 고정했다. 정적 분석 문제 0건, 전체 Flutter 회귀 1,355건, Web Release와 Wasm 사전검사가 통과했다. 로컬 Release를 320×568·1440×900 실제 브라우저에서 열어 한·영 전환과 지도 진입, 요청 자산 HTTP 200, 콘솔 경고·오류 0건을 확인했다.
2026-08-24 | 숨은 스위치 실루엣을 공통 미스터리 상자로 교체했다. 프로젝트 상자 스타일을 참고한 1254px RGBA `?` 상자를 생성하고 `visualState: hidden` 전체에 재사용하며, 공개 전 이름·설명·접근성 레이블도 기믹 종류를 노출하지 않도록 분리한다. 조건 달성 뒤에는 실제 스위치 스프라이트와 안내로 전환한다.
2026-08-24 | 미스터리 상자를 공통 숨은 기믹 계약으로 확장했다. 공개 트리거의 `linkId`가 임의 ID의 숨은 대상을 가리키며, `hidden → mystery_opening → revealed` 물리 애니메이션과 전용 소리·진동을 연결했다. 공개 중에는 HUD·접근성·발견 문구가 정체를 먼저 말하지 않고, 모션 감소·강한 섬광 설정을 존중한다. 5개 화면 크기의 개방 중간 Golden과 임의 ID·순서·되감기 회귀를 추가한다.
2026-08-26 | 3~N중 추돌 끊김을 프레임 단위로 재감사했다. 같은 프레임의 Web 충돌 피드백을 최중요 신호 하나로 합성하고 텔레메트리를 프레임당 4건으로 분산했으며, 연쇄 위치·변형을 거리 시간축으로 통일했다. 동시 시각 효과는 최근 사건만 남기고, 상자·돌·젤리의 두께·외곽선·표면광을 계획 화면 `ui.Picture` 캐시로 미리 기록해 5중 추돌에서 프레임당 고해상도 본체 명령이 반복 생성되지 않게 했다. 전체 직렬 1,476건과 후속 캐시 관련 69건, 생산 40패턴, Wasm 빌드 및 390×844 Chromium headless 3회 검증을 통과했다.
