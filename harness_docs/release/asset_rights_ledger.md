...
- `assets/generated/nav-helm-v1.png`, `nav-stage-map-v1.png`, `nav-exit-door-v1.png`, `nav-replay-v1.png`, `nav-daily-challenge-v1.png`, `nav-activities-v1.png`, `hint-key-v1.png`, `hint-lantern-v2.png`: 시작·지도·스테이지 나가기·리플레이·오늘의 도전·활동 묶음·힌트의 장식형 이모지와 일반 기호를 대체하는 프로젝트 전용 RGBA 삽화다. 나가기는 지도 정보 버튼과 즉시 구분되도록 열린 선실 문과 바깥쪽 화살표를 결합했다. 등불 v2는 24~40px 표시에서 깨져 보이던 미세 장식과 광점을 제거하고 큰 발광 몸체·손잡이·받침만 남긴 소형 표시 전용 자산이다. 기존 프로젝트 삽화만 내부 스타일 참조로 사용했으며 글자·로고·워터마크·제3자 이미지를 입력하지 않았다.
